...
export async function handleCommand(m) {
  try {
    ...
    if (!isOwner && !global.db.data.users[m.sender]?.approved && !global.db.data.settings[m.conn.user.jid]?.approveList?.includes(m.sender)) {
      ...
      // إضافة المستخدم إلى قائمة الانتظار للموافقة
      if (!global.db.data.settings[m.conn.user.jid].approveList) {
        global.db.data.settings[m.conn.user.jid].approveList = [];
      }
      global.db.data.settings[m.conn.user.jid].approveList.push(m.sender);
      await global.db.write();

      // إرسال إشعار إلى المالكين للموافقة
      for (let jid of global.owner) {
        ...
        if (data.exists) {
          await conn.sendMessage(data.jid, {
            ...
            mentions: [m.sender],
            buttons: [
              { buttonId: `.مصادقة ${m.sender}`, buttonText: { displayText: "APPROVE ✅️" }, type: 1 }
            ]
          });
        }
      }
    ...
    // إذا لم يتم الموافقة على المستخدم بعد
    if (!isOwner && !global.db.data.users[m.sender]?.approved && global.db.data.settings[m.conn.user.jid]?.approveList?.includes(m.sender)) {
      return conn.sendMessage(m.chat, { text: "*『❤️‍🩹』طلبك لا يزال قيد المراجعة، يرجى الانتظار『❤️‍🩹』*", mentions: [m.sender] });
    }
    ...
    for (let [name, plugin] of global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        // التحقق من حالة الموافقة قبل تنفيذ الأمر
        if (!isOwner && !global.db.data.users[m.sender]?.approved) {
          return conn.sendMessage(m.chat, { text: "*『❤️‍🩹』لم تتم الموافقة على طلبك بعد، يرجى الانتظار『❤️‍🩹』*", mentions: [m.sender] });
        }

        // تنفيذ الأمر
        try {
          await plugin.call(this, m, { match, usedPrefix, noPrefix, _args, args, command, text, conn, participants, groupMetadata, user, bot, isOwner, isRAdmin, isAdmin, isBotAdmin, chatUpdate, __dirname: ___dirname, __filename });
        } catch (e) {
          console.error(e);
          for (let jid of global.owner) {
            let data = (await conn.onWhatsApp(jid))[0] || {};
            if (data.exists) {
              await conn.sendMessage(data.jid, {
                text: `*『⛔️』حدث خطأ『⛔️』*\n\n*『🚨』الأمر* : *_${name}_*\n\n*『👤』المستخدم* : *_${m.sender}_*\n\n*『📩』الرسالة* : *_${m.text}_*\n\n*『🖲』الخطأ* : \`\`\`${format(e)}\`\`\`\n\n*『⚙️』من فضلك قم بابلاغ سنباي عن الخطأ مستخدما الامر "تقرير"『⚙️』*`,
                mentions: [m.sender]
              });
            }
          }
