...
import usersData from './function/database/usersData.js';
import chatsData from './function/database/chatsData.js';
import settingsData from './function/database/settingsData.js';
...
const { proto, areJidsSameUser, generateWAMessage, getAggregateVotesInPollMessage } = (await import('@adiwajshing/baileys')).default;
...
// دالة handleCommand
export async function handleCommand(m) {
    try {
        ...
        if (!isOwner && !global.db.data.users[m.sender].approved && !global.db.data.settings[m.conn.user.jid].approveList.includes(m.sender)) {
            ...
            for (let jid of global.owner) {
                ...
                if (data.exists) await conn.sendButton(data.jid, aptext, "_*『❤️‍🩹』DONT GIVE UP SENPAI『❤️‍🩹』*_", 'https://i.ibb.co/h7309Rm/113252844-p0-master1200.jpg', [["APPROVE ✅️", `.مصادقة ${m.sender}`]], null, null);
            }
            return;
        }
        ...
        for (let [name, plugin] of global.plugins) {
            ...
            if (typeof plugin.handleMessage == 'function') {
                if (await plugin.handleMessage.call(this, m, { conn, usersData, chatsData, settingsData, participants, groupMetadata, user, bot, isOwner, isRAdmin, isAdmin, isBotAdmin, chatUpdate, __dirname: ___dirname, __filename })) continue;
            }
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                m.plugin = name;
                m.isCommand = true;
                const bot = global.db.data.settings[m.conn.user.jid];
                if (bot.majnonOnly && bot.majnonOnlyNoti && !isOwner) return conn.sendJeen(m.chat, "*『❤️‍🩹』جين في وضع المطور『❤️‍🩹』*\n*『🌺』يحصل هذا عندما اكون قيد الصيانة الرجاء الانتظار『🌺』*");
                if (bot.yamadaOnly && !bot.yamadaOnlyNoti && !isOwner) return;
                if (global.db.data.users[m.sender].banned) return conn.sendMarian(m.chat, "*『❌️』تم حظرك من استخدام جين『❌️』*\n*『❤️‍🩹』تواصل مع سنباي للاستفسار『❤️‍🩹』*");
                if (m.isGroup && global.db.data.chats[m.chat].banned) return conn.sendMarian(m.chat, "*『❌️』تم حظر هذه المجموعة من استخدام ماريان『❌️』*\n*『❤️‍🩹』تواصلو مع سنباي للاستفسار『❤️‍🩹』*");
                if (plugin.owner && !isOwner) return conn.sendMarian(m.chat, "*『❤️‍🩹』لا تمتلك صلاحية استخدام هذا الامر『❤️‍🩹』*\n*『☁️』هذا الامر خاص بسنباي『☁️』*");
                let extra = { usersData, chatsData, settingsData, match, usedPrefix, noPrefix, _args, args, command, text, conn, participants, groupMetadata, user, bot, isOwner, isRAdmin, isAdmin, isBotAdmin, chatUpdate, __dirname: ___dirname, __filename };
                try {
...
async function getMessage(key){
        ...
        return {
            conversation: "My name is Jeen Ai"
        }
    }
